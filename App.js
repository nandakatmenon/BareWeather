...
const openWeatherKey = `849338767c0e95025b5559533d26b7c4`;
...
const App = () => {
  ...
  const loadForecast = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Location access was denied');
    }
    ...
    if(!response.ok) {
      Alert.alert(`There was an error in retrieving the weather data: ${data.message}`); 
    } else {
      setForecast(data);
    }
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView 
        refreshControl={
          ...
          />}
      >
        <Text style={styles.title}>BareWeather</Text>
